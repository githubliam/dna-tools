...
import (
	...
	ocommon "github.com/dnaproject2/DNA/common"
...
func AssignDnaIDsToRole(ontSdk *sdk.DNASdk) bool {
	...
	assignDnaIDsToRoleParam := new(AssignDnaIDsToRoleParam)
	err = json.Unmarshal(data, assignDnaIDsToRoleParam)
	if err != nil {
		log4.Error("json.Unmarshal failed ", err)
		return false
	}
	time.Sleep(1 * time.Second)
	user1, ok := common.GetAccountByPassword(ontSdk, assignDnaIDsToRoleParam.Path1)
	if !ok {
		return false
	}
	ok = assignDnaIDsToRole(ontSdk, user1, utils.GovernanceContractAddress, "TrionesCandidatePeerOwner", assignDnaIDsToRoleParam.Dnaid)
	if !ok {
		return false
	}
	common.WaitForBlock(ontSdk)
...
func AssignDnaIDsToRoleAny(ontSdk *sdk.DNASdk) bool {
	...
	assignDnaIDsToRoleAnyParam := new(AssignDnaIDsToRoleAnyParam)
	err = json.Unmarshal(data, assignDnaIDsToRoleAnyParam)
	if err != nil {
		log4.Error("json.Unmarshal failed ", err)
		return false
	}
	time.Sleep(1 * time.Second)
	user1, ok := common.GetAccountByPassword(ontSdk, assignDnaIDsToRoleAnyParam.Path1)
	if !ok {
		return false
	}
	contractAddress, err := common.GetAddressByHexString(assignDnaIDsToRoleAnyParam.ContractAddress)
	if err != nil {
		log4.Error("getAddressByHexString failed ", err)
		return false
	}
	ok = assignDnaIDsToRole(ontSdk, user1, contractAddress, assignDnaIDsToRoleAnyParam.Role, assignDnaIDsToRoleAnyParam.Dnaid)
	if !ok {
		return false
	}
	common.WaitForBlock(ontSdk)
...
func BlackNode(ontSdk *sdk.DNASdk) bool {
	data, err := ioutil.ReadFile("./params/ForceQuitNode.json")
	if err != nil {
		log4.Error("ioutil.ReadFile failed ", err)
		return false
	}
...
func CommitDpos(ontSdk *sdk.DNASdk) bool {
	data, err := ioutil.ReadFile("./params/SwitchEpoch.json")
	if err != nil {
		log4.Error("ioutil.ReadFile failed ", err)
		return false
	}
